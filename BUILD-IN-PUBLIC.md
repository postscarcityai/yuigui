...
- Plain words, short sentences, no em dashes, no hype vocabulary. Written for a curious outsider, not the fleet. Entries, Thoughts and briefs follow [Yui's values](spec/VALUES.md) (yuigui.com/developers/values): one idea per page, a real title, say what is being done, draw it rather than describe it.
...
Before pushing, `npm run og` from `site/` (SITE-48) must pass: every `/s/<id>` needs a committed `public/og/screens/<id>.jpg` and no private name in its agent or title (CI runs it on every push too); `--base https://www.yuigui.com` after the deploy checks each capture answers 200.
