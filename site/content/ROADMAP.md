...
- YUI-104 (backlog, Chris Sep 25: "simple SVG style vector graphics"): shapes that move. A small set of geometric and organic shapes, labels and connectors that draw on, grow and move, so an agent explains an idea in a few lines instead of generating an image. It sits in the chat with a caption, not full screen. Spec and playground first, then the app. Builds on sketch (YUI-83, YUI-84).
