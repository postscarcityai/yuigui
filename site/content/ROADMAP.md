...
- **On phones:** Yui 0.3.1, build 160, on TestFlight since Sep 26. Three fixes from the feedback button: what you were typing stays put, a lesson is one deck (YUI-113), and an ask you answered leaves the war room at once. [See it](/mockups#release-031).
...
- YUI-68 (Chris Sep 25, shipped in build 74): reply to a message, and the gestures around it. Hold a bubble or a card for reactions, Copy, Select text and Reply. A sideways drag, on a bubble or beside it, switches screens (swipe to reply came out Sep 26 from his TestFlight feedback: it blocked that drag, and hold already had Reply). The agent gets the quote with your answer, and tapping the quote on your reply scrolls back to the original. Copying any part of a message (Select text) shipped the same day from his TestFlight feedback.
...
- YUI-14 (shipped in build 155, Sep 26; Chris: "human voice in, text out, fast and smooth"): tap the mic for hands-free. Words show as you speak (SpeechAnalyzer, on the phone), a short pause sends, the answer comes back as text and the mic reopens. Each agent starts with typing or talking. Audio never leaves the phone.
...
- YUI-63 step 1 (shipped in build 82): one working row instead of three dots plus a timer, a working word and the seconds ("Pondering · 12s"). Step 2: the agent sends a few words on what it is doing, with a small progress bar when it knows the steps. The web half is done (Sep 26): `doing "Reading your calendar" 1/3` is in the Yui Lines spec and every parser in the hub, and /playground?demo=working plays a turn. The app half draws it and teaches agents (backlog).
...
- DONE Sep 26, YUI-14 (shipped in build 155): voice input on Apple's on-device SpeechAnalyzer (classic recognizer as an on-device-only fallback), per-agent default of talk vs type, hands-free voice in with text out. Final words ~90 ms after you stop talking; replies fetched every 350 ms while one is owed. Hold to talk still works (builds 57 and 64). Later: the agent's reply streaming token by token (today it lands whole).
...
- YUI-36: optional encrypted sync of tables via the relay (off by default, on-device first per Chris). Step 1, the decision, the spec and a playground mock, shipped Sep 26 ([Encrypted sync](/developers/sync), `/playground?demo=sync`): v1 ships without sync, since v1 is one iPhone and the phone's own backup covers a new phone. The design is ready for when a second Yui device arrives: agent tables only, sealed on the phone, a relay that cannot read them, pairing by QR with no new account, a per-cell conflict rule, and turning it off deletes the relay's copy. No new Yui Lines word. Step 2 waits for a second Yui client.

Dependencies: decided Sep 26, v1 does not need sync ([Encrypted sync](/developers/sync), five open questions for Chris).
...
- YUI-113 (shipped in 0.3.1, build 160, Sep 26; TestFlight feedback Sep 26, Chris: "I want all that to be in one experience"): a lesson is one swipeable deck. A deck page carries a diagram, formula, chart, stat or calc as its picture (all five parsers, playground, app), channel guide v26 asks for one deck, and phones on older builds get the v25 full-screen layout until they update.
...
- YUI-36: optional encrypted sync for agent tables, off by default. Step 1 shipped Sep 26: [Encrypted sync](/developers/sync), `/playground?demo=sync`. Decision: not in v1; step 2 (native, in the app and the relay) waits for a second Yui device.
...
- FLOW-2: a library of components and flows on yuigui.com that people browse and agents can search and "shop". Later, a store. Step 1 done Sep 25: /developers/library (every screen drawn live, every flow as its chart, search in the browser) and /library.json. Step 2 done Sep 26: agents look it up by intent at /api/library?q= and with the yui_library MCP tool (one ranking for the page, the endpoint and the tool; a flow comes back with its Mermaid), channel guide v28 names the saved flows, and the library check runs before every site build. Next: variants agents make, then a store.
