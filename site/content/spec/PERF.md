...
- **Sent at least once, stored once (YUI-107).** The phone keeps a batch until the server answers, so a send can land twice. Each row carries `row_key`, a random uuid made when the row is queued and kept with it in `perf-pending.json`; the table holds `unique (user_id, row_key)` and the phone posts with `on_conflict=user_id,row_key` and `Prefer: resolution=ignore-duplicates`, so a resent row answers 201 and is not stored again (migration `20260926020000_yui_perf_row_key.sql`). Rows from builds before the key have none and land as before.
