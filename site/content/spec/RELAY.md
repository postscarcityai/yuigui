...
- **The host sends only what it draws.** Each preset has a first build that draws it (the commit count of the commit that taught the app's parser, `MIN_BUILD` in `hermes-plugin/yui/compat.py`): `timeline`/`done`/`now`/`next` 69, `game` 71, `sketch`/`row`/`after` 104, the core word `menu` 115, and `shapes`/`shape` 131. A deck page whose picture is shapes, math, a chart, a stat or a calc needs 158; on an older build those move out of the deck. Before a reply goes out, a gated preset becomes plain words in the chat (a sketch's rows as a list, struck `~~` and bold `**` marks) or, inside a `deck` or `plan`, a `page` with the rows as `points`. A `menu` line draws nothing in the chat, so on an older build it is dropped and left out of the turn's line. A null build counts as older than all of them. Each Yui turn also gets a line naming what not to send ("cannot draw sketch yet"). A new preset adds its line to `MIN_BUILD` in the commit that ships it. Other hosts: read `app_build` from `session` and do the same. As of Sep 26 2026 only the Hermes plugin does: OpenClaw, the webhook, the MCP server, A2A, AG-UI and the model bridge pass lines through, so a phone on an older build shows the Update chip where Hermes would have sent plain words. Proposed fix: `session` returns the preset words the oldest phone can draw, next to `app_build`, so every host gates from one table instead of copying `MIN_BUILD`.
