...
3. **All four parsers pass.** JavaScript (`site/lib/yl/yl.mjs`, add it to `PRESETS`), Swift (`Packages/YuiLines` in the app repo), Python (`parsers/python`) and Kotlin (`parsers/kotlin`). Each runs the full suite, and every vector passes on every parser. Update the preset list in the plugin's `flywheel.py` too, so the new word stops showing up as unknown.
