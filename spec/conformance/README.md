...
- `resolved`: each add's props over its preset's defaults, in line order (`resolve` in every hub parser). Pins what a renderer reads, where the parse alone keeps what was written: a `shape` kind is written `Circle` and resolves `circle`. Checked in JS, Python, Kotlin and Rust; the Swift parser has no `resolve` and skips it.
