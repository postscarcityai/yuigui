...
- **Built, on main, not on phones yet:** YUI-106, send and the drawer taps answer faster. It rides the next build.
...
- YUI-112 (backlog, small; seen Sep 26 on the simulator): a timeline opened alone on the full-screen stage, from the shelf, spreads its rows over the whole height, with big gaps and a running row's highlight about 400 pt tall. Rows should keep their own height and the timeline should sit at the top. Rides the next release.
...
**The next build** carries what lands on the app's main branch past 138: so far YUI-106, send and the drawer taps answer faster. [Builds](/changelog#next) lists it. Releases ship as versions, one upload per epic, and Chris picks each one's scope.
